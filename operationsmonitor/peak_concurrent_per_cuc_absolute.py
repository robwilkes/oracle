...
def run(facade, args):
    ...
    for call in sorted(calllog, key=lambda k: k['timestamp']):
        if devices.get(call['ingress_deviceid'].split(',')[0], None) is None:
            ingress_cuc = call['ingress_deviceid'].split(',')[0]
        else:
            ingress_cuc = devices[call['ingress_deviceid'].split(',')[0]]['name'].split('-')[0]

        if devices.get(call['egress_deviceid'].split(',')[0], None) is None:
            egress_cuc = call['egress_deviceid'].split(',')[0]
        else:
